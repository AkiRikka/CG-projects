...
void ShadingTutorial::initPhongShader() {
    ...
    const char* fsCode = R"(
        ...
        vec3 calcDirectionalLight(vec3 normal, vec3 viewDir) {
            ...
            vec3 spectular = spec * material.ks;
            return directionalLight.intensity * directionalLight.color * (diffuse + spectular);
        }
        ...
        vec3 calcSpotLight(vec3 normal, vec3 viewDir) {
            ...
            vec3 reflectDir = reflect(-lightDir, normal);
            vec3 spectular = spotLight.color * pow(max(dot(reflectDir,viewDir),0.0f), material.ns) * material.ks;
            return spotLight.intensity * attenuation * (diffuse + spectular);
        }
        ...
        void main() {
            ...
            vec3 ambient = material.ka * ambientLight.color * ambientLight.intensity;
            vec3 phongShader = ambient + calcDirectionalLight(normal, viewDir) + calcSpotLight(normal, viewDir);
