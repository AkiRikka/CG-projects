...
void Transformation::handleInput() {
    ...
    // --------------------------------------------------
    _deltaTime = glfwGetTime();
    _positions[0] = glm::vec3(-10.0f, 0.0f, 0.0f) + velocity * sin(_deltaTime);
    _rotateAngles[1] = _deltaTime * angulerVelocity;
    _scales[2] = glm::vec3(sin(_deltaTime) * scaleRate + 1.0f);
;
    // --------------------------------------------------
}
...
void Transformation::renderFrame() {
    ...
    for (std::size_t i = 0; i < _bunnies.size(); ++i) {
        ...
        rotation = glm::rotate(rotation, _rotateAngles[i], glm::vec3(0.0f, 1.0f, 0.0f));
        // @scale
        glm::mat4 scale = glm::mat4(1.0f);
        scale = glm::scale(scale, glm::vec3(_scales[i]));
        // ------------------------------------------------
