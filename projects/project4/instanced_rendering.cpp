...
InstancedRendering::InstancedRendering(const Options& options) : Application(options) {
    ...
    glBufferData(GL_ARRAY_BUFFER, _amount * sizeof(glm::mat4), 
                 &_modelMatrices[0], GL_STATIC_DRAW);
    
    GLsizei vec4Size = sizeof(glm::vec4);
    for (int i = 0; i < 4; i++)
    {
        glEnableVertexAttribArray(i + 3);
        glVertexAttribPointer(i + 3, 4, GL_FLOAT, GL_FALSE, 4 * vec4Size, reinterpret_cast<void*>(i * vec4Size));
...
void InstancedRendering::renderFrame() {
    ...
    case RenderMode::Instanced:
        ...
        // glDraw...(...);
        glBindVertexArray(_asternoid->getVao());
		glDrawElementsInstanced(GL_TRIANGLES, static_cast<GLsizei>(_asternoid->getIndices().size()), 
                                GL_UNSIGNED_INT, 0, _amount);
		glBindVertexArray(0);
        // ---------------------------------------------------------
